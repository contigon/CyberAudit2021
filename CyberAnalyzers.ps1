<#	
	...
	===========================================================================
	.DESCRIPTION
		Cyber Audit Tool - Audit Data Analyzers tools 
#>
...
"@
    ...
    if ($excelVer) {
        ...
        Write-Host $help -ForegroundColor Yellow
        Start-Process iexplore $ACQ
        $null = Read-Host "Press [Enter] if all files are located in $ACQ (or Ctrl+C to quit)"
...
"@
    write-host $help -ForegroundColor Yellow
    $userInput = Read-Host "Press [B] to browse for the location of pwned-passwords-ntlm-ordered-by-hash-v5.txt file"
    if ($userInput -eq "B") {
        Import-Module DSInternals
        $bk = Get-BootKey -SystemHivePath $ACQ\SYSTEM
...
function Initialize-OfflineNTDS {
...
do {
    ...
    switch ($userInput) {
        ...
        6 { Show-Statistics }
        #DSInternals
        7 { Invoke-DSInternals }
        #appInspector
        8 { Invoke-AppInspector }
        #Remediation Efforts and Risk Charts
        9 { Export-RiskCharts }
        #Offline NTDS
        10 { Initialize-OfflineNTDS }
        #End Menu
    }
    Clear-Host
}
